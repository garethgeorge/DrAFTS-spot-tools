...
parser.addArgument(
  ["--intervalsFile"],
  {
    help: "a CSV file formatted as [region, az, start, stop] of intervals of continuous date" + 
...
const runExperiment = async (startTime, endTime) => {
  console.log("running experiment over interval ", startTime, " - ",  endTime);

  const daysBetweenOldestAndNewestRecord = 
      ((endTime.getTime() - startTime.getTime()) / msOneDay);
  ...
  const intervalStart = startTime; // start fetching data, includes backlog
  const intervalEnd = endTime; // end fetching data
  ...
  const results = {}
  results.pgraph = args.includePGraph ? pgraphArray : null;
...
(async () => {
  ...
  if (args.intervalsFile) {
    ...
    for await (const line of streamLineByLine(fs.createReadStream(args.intervalsFile))) {
      ...
      if (region === args.region && az === args.az && args.insttype === insttype) {
    ...
    result = await asyncMap(intervals, async ({start, stop}) => {
      console.log("running experiments on interval ", start, " - ", stop);
      try {
        return await runExperiment(start, stop);
      } catch (e) {
        console.log(e);
        return {
          error: e.toString()
    ...
    debug("no args.intervalFile, running experiment on default interval: starting at current date and working backwards");
    ...
    result = runExperiment(oldestHistoryRecord.Timestamp, newestHistoryRecord.Timestamp);
  }
